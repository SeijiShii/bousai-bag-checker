# 改修 #001 ドキュメントインデックス — Clerk SDK 実 wiring

**issue / slug**: 001 / clerk-sdk-wiring
**実施日**: 2026-05-28
**状態**: 設計完了 (実装は /flow:tdd で)

<!-- AUTO-GENERATED:BEGIN status -->

## ファイル一覧

| 番号 | ファイル | 種別 | 最終更新 |
|---|---|---|---|
| 001 | 001_REVISE_SPEC.md | 変更仕様書 | 2026-05-28 |
| 002 | 002_REVISE_PLAN.md | 変更計画書 | 2026-05-28 |
| 003 | 003_REVISE_UNIT_TEST.md | 単体テスト計画 | 2026-05-28 |
| 004 | 004_REVISE_E2E_TEST.md | E2E テスト計画 | 2026-05-28 |
| 101 | (未生成) | 実装レポート | /flow:tdd で生成予定 |
| 102 | (未生成) | 単体テストレポート | /flow:tdd で生成予定 |

## 関連
- 親機能 INDEX: `../INDEX.md`
- 基準 SPEC: `../001__shared_auth_SPEC.md`
- 並行 revise (loop で順次): `../../billing/revise_*_*_stripe-sdk-wiring/`, `../../notification/revise_*_*_resend-sdk-wiring/` (予定)
- 関連 perspective: O35 (inject seam pattern), SEC-001 (認可基盤)

<!-- AUTO-GENERATED:END status -->

<!-- user-edit-start -->
<!-- user-edit-end -->
