...
4. **買い物 TODO リストで買い替えを管理する** — 期限切れ・不足分を買い物 TODO リストに起こし、買ったものをチェックして購入状況(買った/未購入)を管理する。**この機能を 100 円買い切りでアンロック**(本サービスの唯一の有料機能)
...
- 期限リマインド(メール + アプリ内通知)
- 季節点検モード
- 買い物 TODO リスト(購入状況の TODO 管理。**100 円買い切りで解放する有料機能**)
- 100 円単発買い切り課金(one-time。継続課金なし。買い物 TODO リスト機能のアンロックに対して)
- フィードバックウィジェット
...
| docs/shopping-list/ | 期限切れ/不足分から買い物 TODO リスト生成 + 購入状況管理(UC4、**100円有料機能**) | 買い物 TODO リスト画面・購入チェック・課金ゲート | inventory, _shared/billing | 4 | ❌ |
...
| docs/_shared/billing/ | 課金基盤(100円 one-time 買い切り) | Stripe 単発決済、買い物 TODO リストのアンロック、特商法導線 | _shared/db, _shared/auth | 3 | ❌ |
...
P4: inspection (←inventory, notification), shopping-list (←inventory, billing)
```
循環依存: なし。
...
| 課金 | Stripe | Paddle | 100円 one-time 買い切り、従量手数料のみ | 売上の ~3.6% ※ 同上 |
...
**本プロジェクトのレンジ**: 個人・無料枠。商用化想定なし、**無料枠厳守、上限到達時は §4.3 代替候補に切替判断**。Stripe 手数料のみ売上連動(100 円単発買い切り × 購入数。1 件あたり Stripe 手数料 ~3.6% + 固定費で実質手取りは数十円規模、収益目的ではなく機能価値の対価)。
...
#### 4.6.4 収益指標
本 PJ は無料枠 + 100円単発買い切りのため MRR/ARR/Churn 追跡は不要。買い切り購入数 × 100円 の累計のみ任意記録(Stripe ダッシュボード)。
...
3. 課金停止(単発買い切りなので継続課金なし、Stripe one-time のみ)
...
#### 4.8.2 製品内グロース設計
- シェアしたくなる成果物: 「点検完了で全部グリーン」の安心スクショ。**ただし控えめな導線**(シェアしなくても無料機能は全部使える。買い物 TODO リストのみ 100 円有料)
...
- **billing**(課金状態): user_id, stripe_customer_id, unlocked_features(100 円買い切りで `shopping_todo` 等を付与), last_payment_at
- **shopping_item**(買い物 TODO、**有料機能**): id, user_id, item_id?(元品目への参照、任意), name, reason(expired/insufficient/manual), is_bought(購入済みフラグ), bought_at, created_at
...
買い物 TODO リスト(課金済ユーザー) → 期限切れ/不足 item を抽出 → shopping_item に起こす → 購入チェックで is_bought 更新
...
| Stripe | 100円 one-time 買い切り課金 | SDK + Webhook | API キー + Webhook 署名 |
...
| 2026-05-26 | 課金=**100 円単発買い切り(one-time)**。売る機能=**買い物 TODO リスト**(期限切れ/不足から TODO 生成 + 購入チェック管理)。PDF・PWYW・品目テンプレ拡張は廃止。唯一の有料機能 | ユーザー指示「課金は100円単発買い切り / PDFではない / 買ったことを TODO 管理 / 買い物 TODO リストを100円で売るイメージ」 | §1.1, §1.2, §1.3, §4.4, §5, §9.4, _shared/billing, shopping-list | [D20260526-024](./AI_LOG/D20260526_002_resume_continuous.md#decisions) |
...
> 公開 + 有償(100円単発買い切り)PJ。最低 §9.1 プライバシーポリシー必須、有償時 §9.4 特商法表記必須。
...
| 特定商取引法に基づく表記 | ✅ | 未作成 | `/legal/specified-commercial-transactions` | 100円単発買い切り課金公開前に必須 |
...
- **価格**: 100 円(税込)の単発買い切り(買い物 TODO リスト機能のアンロック)。継続課金なし
- **支払方法**: クレジットカード等(Stripe)/ **引渡**: 決済完了後ただちに機能アンロック / **返品・キャンセル**: デジタル機能の性質上、決済後の返金は原則不可(その旨を明記)
