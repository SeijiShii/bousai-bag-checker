...
4. **買い物 TODO リストで買い替えを管理する** — 期限切れ・不足分を買い物 TODO リストに起こし、買ったものをチェックして購入状況(買った/未購入)を管理する。**無料機能**(ペイウォールなし)
6. **投げ銭で応援する(任意)** — 社会善アプリのため機能は全て無料。気に入ったら 100 円の投げ銭で運営を任意支援できる(機能アンロックは伴わない)
5. **フィードバックを送る** — 好き嫌い(👍/👎)+ バグ報告ウィジェット(運用品質シグナル、O40)

### 1.2 スコープ
**含むもの (MVP / v1)**:
- 単一ユーザー(1 世帯 = 1 アカウント)の品目管理
- ゲスト開始 → クラウド同期時に Clerk 認証(段階的認証)。投げ銭はログイン不要
- 期限リマインド(メール + アプリ内通知)
- 季節点検モード
- 買い物 TODO リスト(購入状況の TODO 管理。**無料**)
- 100 円の投げ銭(任意の支援。社会善アプリのためペイウォールにせず、**機能アンロックは伴わない**。Stripe で任意・ログイン不要・複数回可)
- フィードバックウィジェット
- **多言語対応(UI を 日本語/英語/中国語(簡体)/韓国語 の 4 言語)** — 在留外国人・訪日者を含む防災備蓄ニーズに対応(日本の自治体防災情報の標準 4 言語)。react-i18next + 言語切替 UI。ソース言語=日本語。**法務長文(プライバシー/利用規約/特商法)は日本語を正本**とし(特商法は日本法固有)、各法務ページに「正本は日本語」を注記、UI チローム(ラベル/ボタン/メッセージ/空状態/エラー/入口説明)を 4 言語化(D-100〜103)
...
| docs/shopping-list/ | 期限切れ/不足分から買い物 TODO リスト生成 + 購入状況管理(UC4、**無料**) | 買い物 TODO リスト画面・購入チェック | inventory | 4 | ❌ |
...
| docs/_shared/i18n/ | 多言語基盤(4 言語) | react-i18next 設定・ロケールカタログ(ja/en/zh-Hans/ko)・ロケール検出(ブラウザ+手動)・言語切替 UI・日付/数値整形・全 UI 文字列のキー化。法務長文は JA 正本(対象外) | (なし) | 1 | ✅ |
...
| docs/_shared/billing/ | 投げ銭基盤(100円の任意支援) | Stripe 単発決済(投げ銭)、支援記録、**機能アンロックなし** | _shared/db | 2 | ❌ |
...
```
P1: _shared/db (基盤), _shared/ui (基盤), _shared/i18n (基盤、多言語、retrofit)
P2: _shared/auth (←db, 基盤), _shared/notification (←db, 基盤), _shared/legal, _shared/service-info (←db), _shared/billing (←db, 投げ銭)
P3: inventory (←db, auth, ui, 基盤), feedback (←db, auth)
...
| 国際化 (i18n) | UI を 4 言語(ja/en/zh-Hans/ko)で提供。ロケール検出(ブラウザ Accept-Language + 手動切替、選択は永続化)。日付/数値はロケール整形。法務長文は JA 正本(各ページ「正本は日本語」注記) | 在留外国人・訪日者の防災ニーズ。react-i18next、ソース=日本語 |
...
| DB | 品目・通知設定・投げ銭記録 | Neon (Postgres) + Drizzle |
| 認証 | ゲスト→段階的認証 | Clerk |
| 通知 | 期限リマインド配信 | Resend (メール) + アプリ内通知 |
| 投げ銭 | 100円の任意支援 | Stripe |
...
| 投げ銭 | Stripe | — | 100円の任意支援(投げ銭)、従量手数料のみ | 支援額の ~3.6%+固定費 ※ 同上 |
...
**本プロジェクトのレンジ**: 個人・無料枠。商用化想定なし、**無料枠厳守、上限到達時は §4.3 代替候補に切替判断**。Stripe 手数料のみ(100 円投げ銭 × 支援数。1 件あたり Stripe 手数料 ~3.6% + 固定費で実質手取りは数十円規模、収益目的でなく社会善アプリへの任意支援)。
...
#### 4.6.4 収益指標
本 PJ は無料枠 + 100円投げ銭(任意支援)のため MRR/ARR/Churn 追跡は不要。投げ銭の累計のみ任意記録(Stripe ダッシュボード)。
...
3. 投げ銭受付停止(任意支援・継続課金なし。未提供の対価はないため精算不要)
...
#### 4.8.2 製品内グロース設計
- シェアしたくなる成果物: 「点検完了で全部グリーン」の安心スクショ。**ただし控えめな導線**(シェアしなくても全機能が無料で使える。100 円の投げ銭はあくまで任意の応援)
...
### 5.1 主要エンティティ
- **user**(Clerk 連携、ゲスト→アカウント): id, clerk_user_id(plan 区分は廃止=全機能無料)
...
- **donation**(投げ銭記録): id, user_id?(ゲスト投げ銭可で nullable), stripe_payment_id, amount(default 100), created_at
- **shopping_item**(買い物 TODO、**無料機能**): id, user_id, item_id?(元品目への参照、任意), name, reason(expired/insufficient/manual), is_bought(購入済みフラグ), bought_at, created_at
...
買い物 TODO リスト → 期限切れ/不足 item を抽出 → shopping_item に起こす → 購入チェックで is_bought 更新
投げ銭 → Stripe Checkout(100円) → webhook(署名検証)→ donation 記録(機能アンロックなし)
...
| Stripe | 100円 投げ銭(任意支援) | SDK + Webhook | API キー + Webhook 署名 |
...
| 2026-05-27 | **多言語対応 (i18n) を追加**: UI を ja(正本)/en/zh-Hans/ko の 4 言語化。ライブラリ=react-i18next。翻訳スコープ=UI チロームのみ 4 言語、法務長文は JA 正本(特商法は日本法固有、各法務ページ「正本は日本語」注記)。新 横断 `_shared/i18n` (優先度1 基盤、retrofit) | ユーザー指示「このアプリは多言語対応したい」+ 設計判断 4 件 | §1.2, §1.3.2/4, §3(国際化 NFR), §9.2 | [D20260527-101](./AI_LOG/D20260527_041_concept_update_i18n.md#decisions) |
...
- **検出根拠**: API 入力スキーマ (Zod) 一元化方針が未記載。買い物 TODO リストの共有/エクスポート (UC4) における CSV インジェクションエスケープ未設計。photo_url 取り扱い時の SSRF 検討
...
> **多言語 UI と法務の関係 (2026-05-27)**: UI を 4 言語化しても**サービスは引き続き日本国内向け**(在留外国人・訪日者向けの言語アクセシビリティであり、海外向け事業展開ではない)。**法務長文(プラポリ/利用規約/特商法)は日本語を正本**とし、各法務ページに「正本は日本語です」を明示。zh/ko/en での法務翻訳は提供せず JA にフォールバック(特商法は日本法固有、誤訳・責任リスク回避)。GDPR 等の posture は変わらない。

### 9.3 書類作成方針
- 作成手段: テンプレ採用(Termly 等)+ 自前ドラフト。配置: `_shared/legal/` に原稿、公開時 `/legal/*`
- 公開導線: フッタリンク + 投げ銭前の確認(任意支援である旨)+ 免責(防災情報)表示

### 9.4 投げ銭(任意支援)に関する表記(日本国内)
- 100 円の投げ銭は**対価なき任意の支援**(機能アンロックや商品・役務の提供を伴わない)。このため特定商取引法の「通信販売(商品・役務の販売)」には原則該当しない。
- ただし決済を扱うため、運営者情報(事業者/連絡先)+「**支援は任意であり、支払っても追加機能は得られない/返金は原則不可**」を投げ銭導線・フッタに明示する。
- 支払方法: クレジットカード等(Stripe Checkout)。継続課金なし(単発)。
- 念のため、決済導線の表現次第で特商法該当性が変わり得るため、公開前に最終確認する(§8 で要確認とする)。
...
| 2026-05-27 | 多言語対応 (i18n) スコープ追加: UI 4 言語(ja/en/zh-Hans/ko)、react-i18next、法務は JA 正本。§1.2/§1.3.2/§1.3.4/§3(国際化)/§9.2 反映、_shared/i18n 新設 | /flow:concept (UPDATE, ユーザー指示) |
