...
describe("ItemList", () => {
  ...
  it("UC1-S2: 0件は EmptyState + 追加 CTA", () => {
    ...
    expect(
      screen.getByText(/まだ品目が登録されていません/),
    ).toBeInTheDocument();
  ...
  it("UC3: 削除ボタンで onDelete(id)", () => {
    ...
    render(
      <ItemList items={[item({ id: "x", name: "水" })]} onDelete={onDelete} />,
    );
...
describe("ItemForm", () => {
  ...
  it("UC2-S2: freshness=電池(交換目安)で月数入力に切替", () => {
    ...
    fireEvent.change(screen.getByLabelText("管理のしかた"), {
      target: { value: "replace_guide" },
    });
