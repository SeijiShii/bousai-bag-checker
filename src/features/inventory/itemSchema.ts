...
// message は i18n キー (spec-review R3)。表示層 (ItemForm) で t(message) する。Zod は t に非依存 (テスタビリティ)。
export const itemInputSchema = z
  .object({
    name: z.string().min(1, "errors.nameRequired").max(200),
    category: z.enum(ITEM_CATEGORIES),
    quantity: z.number().int().min(0, "errors.quantityInvalid").default(1),
  ...
  .refine((v) => v.freshnessType !== "expiry" || !!v.expiresAt, {
    message: "errors.expiresRequired",
    path: ["expiresAt"],
  })
  .refine((v) => v.freshnessType !== "replace_guide" || !!v.replaceMonths, {
    message: "errors.replaceMonthsRequired",
    path: ["replaceMonths"],
  });
