...
export function SettingsScreen() {
  ...
  const { data: settings, loading, reload } = useBackendData(() => backend.getSettings());
    ...
    <section aria-labelledby="settings-heading" className="flex flex-col gap-4">
      ...
      {loading || !settings ? (
        ...
        <Card className="flex flex-col gap-3">
          ...
          <Field
            ...
            onChange={(e) => patch({ leadDays: Number(e.target.value) || settings.leadDays })}
          />
        </Card>
      )}
      ...
      <Card className="flex flex-col gap-2">
        ...
        <p className="text-sm text-text-muted">
          すべての機能は無料で使えます。100 円の投げ銭で開発を応援できます（任意・機能は変わりません）。
        </p>
