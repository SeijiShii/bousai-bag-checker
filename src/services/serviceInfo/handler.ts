...
import { collectMetrics, type MetricsSource, type ServiceInfo } from './collectMetrics';

type AnyDb = PgDatabase<PgQueryResultHKT, typeof schema>;

export interface ServiceInfoResult {
  status: 200 | 401 | 429;
  body: ServiceInfo | { error: string };
}
...
export interface ServiceInfoDeps {
  ...
  /** 共有シークレット (SERVICE_INFO_TOKEN、サーバー専用)。service-hub 契約。 */
  expectedToken: string;
}

/**
 * GET /api/service-info ハンドラ (SEC-004 認可スコープ + PII 非混入)。
