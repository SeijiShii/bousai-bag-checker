...
async function gotoShopping(page: import('@playwright/test').Page) {
  await page.goto('/');
  await page.getByRole('navigation', { name: 'メインナビ' }).getByRole('button', { name: /買い物/ }).click();
  await expect(page.getByRole('heading', { name: '買い物', level: 2 })).toBeVisible();
}

test('UC4-S1: 生成で期限切れ/近い品目が TODO 化', async ({ page }) => {
  await gotoShopping(page);
  await page.getByRole('button', { name: 'リストを生成' }).click();
...
test('UC4-S2: 購入チェックで購入済になる', async ({ page }) => {
  await gotoShopping(page);
  await page.getByRole('button', { name: 'リストを生成' }).click();
  const toggle = page.getByRole('checkbox', { name: 'モバイルバッテリーを購入済みにする' });
  await expect(toggle).not.toBeChecked();
  await toggle.check();
  await expect(page.getByRole('checkbox', { name: 'モバイルバッテリーを購入済みにする' })).toBeChecked();
});
